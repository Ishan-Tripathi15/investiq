...
export class ProfileController {
  ...
  @Get('kyc/health') kycHealth(){return this.profile.kycHealth();}
  @Post('kyc/start') startKyc(@Req() req:AuthenticatedRequest){return this.profile.startKyc(req.user!.id);}
  @Get('kyc/:reference') kycStatus(@Req() req:AuthenticatedRequest,@Param('reference') reference:string){return this.profile.kycStatus(req.user!.id,reference);}
}

...
export interface FundamentalsProvider { readonly name: string; health(): Promise<{ available: boolean }>; fundamentals(symbol: string): Promise<FundamentalsResponse>; }
export class UnconfiguredFundamentalsProvider implements FundamentalsProvider {
  readonly name = 'unconfigured';
  async health() { return { available: false }; }
  async fundamentals(symbol: string): Promise<FundamentalsResponse> { return { symbol: symbol.toUpperCase(), available: false, data: null, source: null, message: 'No verified fundamentals provider is configured.' }; }
}

type Json = Record<string, unknown>;
type InternalPeriod = { fiscalDate: string; [key: string]: string | number | undefined };
function num(value: unknown): number | undefined { const n = typeof value === 'number' ? value : Number(value); return Number.isFinite(n) ? n : undefined; }
function nested(obj: Json | undefined, ...keys: string[]): number | undefined { let current: unknown = obj; for (const key of keys) { if (!current || typeof current !== 'object') return undefined; current = (current as Json)[key]; } return num(current); }
function first(...values: Array<number | undefined>): number | undefined { return values.find((v) => v !== undefined); }
function pct(value: number | undefined): number | undefined { if (value === undefined) return undefined; return Math.abs(value) <= 2 ? value * 100 : value; }
...
export class TwelveDataFundamentalsProvider implements FundamentalsProvider {
  ...
  private async request(path: string, symbol: string, params: Record<string, string> = {}): Promise<Json> {
    const url = new URL(path, this.baseUrl); url.searchParams.set('symbol', symbol.toUpperCase()); url.searchParams.set('apikey', this.apiKey!); for (const [key, value] of Object.entries(params)) url.searchParams.set(key, value);
    const controller = new AbortController(); const timeout = setTimeout(() => controller.abort(), this.timeoutMs);
    try { const response = await fetch(url, { signal: controller.signal }); const body = await response.json() as Json; if (!response.ok || body.status === 'error') throw new Error(String(body.message ?? `Fundamentals provider returned HTTP ${response.status}`)); return body; }
  ...
  async fundamentals(symbol: string): Promise<FundamentalsResponse> {
    ...
    const results = await Promise.allSettled([
      this.request('/statistics', ticker),
      this.request('/income_statement', ticker, { period: 'annual', outputsize: '10' }),
      this.request('/balance_sheet', ticker, { period: 'annual', outputsize: '10' }),
      this.request('/cash_flow', ticker, { period: 'annual', outputsize: '10' }),
    ]);
    const [statsResult, incomeResult, balanceResult, cashResult] = results;
    const statsBody = statsResult.status === 'fulfilled' ? statsResult.value : {};
    const incomeBody = incomeResult.status === 'fulfilled' ? incomeResult.value : {};
    const balanceBody = balanceResult.status === 'fulfilled' ? balanceResult.value : {};
    const cashBody = cashResult.status === 'fulfilled' ? cashResult.value : {};
    ...
    const byDate = new Map<string, InternalPeriod>();

    for (const row of incomeRows) {
      const fiscalDate = String(row.fiscal_date ?? ''); if (!fiscalDate) continue;
      const period: InternalPeriod = { fiscalDate };
      ...
      if (period.revenue && period.revenue !== 0 && period.grossProfit !== undefined) period.grossMarginPct = period.grossProfit / period.revenue * 100;
      if (period.revenue && period.revenue !== 0 && period.ebit !== undefined) period.operatingMarginPct = period.ebit / period.revenue * 100;
      if (period.revenue && period.revenue !== 0 && period.netIncome !== undefined) period.netMarginPct = period.netIncome / period.revenue * 100;
    ...
    const rawSnapshot = {
      ...
      periods: [...byDate.values()].sort((a, b) => a.fiscalDate.localeCompare(b.fiscalDate)) as FinancialPeriod[],
    };
    const snapshot = rawSnapshot as FundamentalSnapshot;
    const endpointErrors = results.filter((result): result is PromiseRejectedResult => result.status === 'rejected').map((result) => result.reason instanceof Error ? result.reason.message : 'Provider endpoint unavailable.');
    const available = incomeRows.length > 0 || Object.keys(stats).length > 0;
    if (!available) return { symbol: ticker, available: false, data: null, source: null, message: endpointErrors[0] ?? 'Verified fundamentals are unavailable.' };
    return {
      symbol: ticker,
      available: true,
      data: snapshot,
      source: { provider: this.name, retrievedAt: new Date().toISOString() },
      ...(endpointErrors.length ? { message: `Partial fundamentals: ${endpointErrors.length} provider endpoint(s) unavailable.` } : {}),
    };
  }
}

export function createFundamentalsProvider(): FundamentalsProvider { return process.env.TWELVE_DATA_API_KEY ? new TwelveDataFundamentalsProvider() : new UnconfiguredFundamentalsProvider(); }
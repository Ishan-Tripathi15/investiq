...
export class AuthService {
  constructor(private readonly sessions: AuthRepository) {}
  private users(): ConfigUser[] {
    const raw = process.env.AUTH_USERS_JSON?.trim();
    if (!raw) throw new ServiceUnavailableException('Authentication users are not configured');
  ...
  async login(username: string, password: string) {
    ...
    if (!user || !verifyPassword(password, user.passwordHash)) throw new UnauthorizedException('Invalid credentials');
    const sessionId = randomUUID();
    const refresh = issueToken(this.publicUser(user), 'refresh', sessionId);
    const exp = verifyToken(refresh, 'refresh').exp;
  ...
  async refresh(refreshToken: string) {
    ...
    if (!claims.sid) throw new UnauthorizedException('Refresh session is invalid');
    const oldHash = hashRefresh(refreshToken);
    const session = await this.sessions.findActive(claims.sid, oldHash);
    if (!session || session.userId !== claims.sub) throw new UnauthorizedException('Refresh session is invalid or expired');
    const user = this.users().find((x) => x.id === claims.sub);
    if (!user) throw new UnauthorizedException('User is no longer active');
    const next = issueToken(this.publicUser(user), 'refresh', claims.sid);
    const nextExp = verifyToken(next, 'refresh').exp;
  ...
  async logout(refreshToken: string) {
    try { const claims = verifyToken(refreshToken, 'refresh'); if (claims.sid) await this.sessions.revoke(claims.sid, hashRefresh(refreshToken)); } catch { /* logout is intentionally idempotent */ }
    return { success:true };
  }

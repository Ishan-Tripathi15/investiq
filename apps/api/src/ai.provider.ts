...
class HttpAiProvider implements AiProvider {
  ...
  async analyze(context: AiAnalysisContext): Promise<AiAnalysisResult> {
    ...
    const schema = { type: 'object', additionalProperties: false, properties: {
      summary: { type: 'string' }, confidence: { type: 'number', minimum: 0, maximum: 1 }, riskLevel: { type: 'string', enum: ['low', 'medium', 'high', 'unknown'] },
      keySignals: { type: 'array', items: { type: 'string' } }, riskFactors: { type: 'array', items: { type: 'string' } }, assumptions: { type: 'array', items: { type: 'string' } },
      invalidationConditions: { type: 'array', items: { type: 'string' } }, citedFeatures: { type: 'array', items: { type: 'string' } },
    ...
    const response = await fetch(this.url, {
      ...
      body: JSON.stringify({ model: this.model, temperature: 0, messages: [
        { role: 'system', content: 'You are InvestIQ financial intelligence. Use only the supplied verified observations. Never invent prices, returns, financial metrics, news, or portfolio facts. Clearly separate historical observations from scenarios. Do not give guaranteed returns or personalized regulated financial advice. Return only the requested structured object.' },
    ...
    const content = choices?.[0]?.message && typeof choices[0].message === 'object' ? (choices[0].message as Record<string, unknown>).content : undefined;
    if (typeof content !== 'string') throw new Error('AI provider returned no structured content');
    let parsed: unknown; try { parsed = JSON.parse(content); } catch { throw new Error('AI provider returned invalid JSON'); }

...
export class TradingReconciliationService {
  ...
  async reconcile(): Promise<TradingReconciliation> {
    ...
    const [account, positions, brokerOrders, executionRequests] = await Promise.all([
      ...
      this.repository.listExecutionRequests(),
    ]);
    ...
    const unmatchedExecutions = completed
      .filter((item) => item.orderId && !brokerOrderIds.has(item.orderId))
      .map((item) => item.orderId as string);

    const warnings: string[] = [];
    if (unmatchedExecutions.length) {
      warnings.push(`${unmatchedExecutions.length} completed local execution record(s) were not returned by the broker order snapshot.`);
    }

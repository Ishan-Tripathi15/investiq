  ...
  return (
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.connection}><View style={[styles.dot, broker?.connected && styles.dotOn]} /><View style={styles.connectionCopy}><Text style={styles.connectionTitle}>{broker?.connected ? `Connected · ${broker.broker}` : 'Execution broker not connected'}</Text><Text style={styles.muted}>{broker?.message ?? 'Connect a supported broker before submitting a live order.'}</Text></View></View>
        <View style={styles.card}>
          <Text style={styles.label}>INSTRUMENT</Text>
          <View style={styles.symbolRow}><TextInput value={symbol} autoCapitalize="characters" onChangeText={setSymbol} onSubmitEditing={loadReference} style={styles.symbolInput} placeholder="NSE/BSE symbol" placeholderTextColor={colors.muted} /><TouchableOpacity onPress={loadReference} style={styles.lookup} disabled={loading}><Text style={styles.lookupText}>{loading ? '…' : 'Quote'}</Text></TouchableOpacity></View>
          <View style={styles.quoteRow}><View><Text style={styles.muted}>Latest available reference</Text><Text style={styles.quote}>{money(referencePrice)}</Text></View><Text style={[styles.quoteSource, !referencePrice && styles.quoteUnavailable]}>{source ? 'VERIFIED' : 'NO QUOTE'}</Text></View>
        ...
        <View style={styles.sideRow}><TouchableOpacity onPress={() => setSide('buy')} style={[styles.sideButton, side === 'buy' && styles.buyActive]}><Text style={[styles.sideText, side === 'buy' && styles.sideActiveText]}>BUY</Text></TouchableOpacity><TouchableOpacity onPress={() => setSide('sell')} style={[styles.sideButton, side === 'sell' && styles.sellActive]}><Text style={[styles.sideText, side === 'sell' && styles.sideActiveText]}>SELL</Text></TouchableOpacity></View>
        <View style={styles.card}>
          <Text style={styles.label}>ORDER TYPE</Text><View style={styles.typeGrid}>{orderTypes.map(item => <TouchableOpacity key={item.key} onPress={() => { setType(item.key); setPreview(null); }} style={[styles.typeButton, type === item.key && styles.typeActive]}><Text style={[styles.typeTitle, type === item.key && styles.typeActiveText]}>{item.label}</Text><Text style={styles.typeHint}>{item.hint}</Text></TouchableOpacity>)}</View>
          <Text style={styles.label}>QUANTITY</Text><TextInput value={quantity} onChangeText={setQuantity} keyboardType="numeric" style={styles.field} placeholder="Quantity" placeholderTextColor={colors.muted} />
        ...
        <View style={styles.previewCard}><View style={styles.previewHeader}><Text style={styles.previewTitle}>Order preview</Text><Text style={styles.day}>DAY</Text></View><Row label="Side" value={side.toUpperCase()} /><Row label="Instrument" value={symbol.trim().toUpperCase() || '—'} /><Row label="Type" value={orderTypes.find(item => item.key === type)?.label ?? type} /><Row label="Quantity" value={numericQuantity ? numericQuantity.toLocaleString('en-IN') : '—'} />{type !== 'market' && <Row label="Limit / trigger" value={type === 'stop_loss' ? money(numericStop) : type === 'stop_limit' ? `${money(numericStop)} / ${money(numericPrice)}` : money(numericPrice)} />}<Row label="Estimated value" value={money(preview?.estimatedValue ?? orderValue)} />{preview && <View style={styles.previewMessage}><Text style={preview.valid ? styles.success : styles.errorText}>{preview.message}</Text>{preview.errors?.map(error => <Text key={error} style={styles.errorText}>• {error}</Text>)}</View>}<TouchableOpacity onPress={buildPreview} style={styles.previewButton} disabled={loading}><Text style={styles.previewButtonText}>{loading ? 'VALIDATING…' : 'Validate & Preview'}</Text></TouchableOpacity><TouchableOpacity onPress={submitOrder} style={[styles.submitButton, !preview?.valid && styles.submitDisabled]} disabled={submitting || !preview?.valid}><Text style={styles.submitText}>{submitting ? 'SUBMITTING…' : 'Submit live order'}</Text></TouchableOpacity><Text style={styles.executionNote}>Submitting sends the order to the configured broker only. If no broker is connected, InvestIQ blocks execution rather than simulating a fill.</Text></View>
...
const styles = StyleSheet.create({
  safe:{flex:1,backgroundColor:colors.background},content:{padding:spacing.lg,paddingBottom:60,gap:spacing.lg},top:{flexDirection:'row',justifyContent:'space-between',alignItems:'center'},back:{color:colors.accent,fontSize:13,fontWeight:'800'},eyebrow:{color:colors.muted,fontSize:9,fontWeight:'900',letterSpacing:1.4},title:{color:colors.text,fontSize:32,fontWeight:'900'},subtitle:{color:colors.muted,fontSize:14,lineHeight:20,marginTop:-9},connection:{flexDirection:'row',gap:11,backgroundColor:colors.surfaceElevated,borderWidth:1,borderColor:colors.border,borderRadius:radius.md,padding:spacing.md},dot:{width:9,height:9,borderRadius:5,backgroundColor:colors.warning,marginTop:4},dotOn:{backgroundColor:colors.accent},connectionCopy:{flex:1,gap:3},connectionTitle:{color:colors.text,fontSize:12,fontWeight:'900'},muted:{color:colors.muted,fontSize:10,lineHeight:16},card:{backgroundColor:colors.surface,borderWidth:1,borderColor:colors.border,borderRadius:radius.lg,padding:spacing.md,gap:spacing.md},label:{color:colors.accent,fontSize:9,fontWeight:'900',letterSpacing:1.2},symbolRow:{flexDirection:'row',gap:8},symbolInput:{flex:1,color:colors.text,backgroundColor:colors.surfaceElevated,borderRadius:radius.sm,padding:13,fontSize:16,fontWeight:'800'},lookup:{backgroundColor:colors.accentSoft,borderRadius:radius.sm,paddingHorizontal:18,justifyContent:'center'},lookupText:{color:colors.accent,fontSize:11,fontWeight:'900'},quoteRow:{flexDirection:'row',justifyContent:'space-between',alignItems:'center'},quote:{color:colors.text,fontSize:24,fontWeight:'900',marginTop:3},quoteSource:{color:colors.accent,fontSize:9,fontWeight:'900'},quoteUnavailable:{color:colors.warning},disclaimer:{color:colors.muted,fontSize:9,lineHeight:14},sideRow:{flexDirection:'row',backgroundColor:colors.surface,borderRadius:radius.md,borderWidth:1,borderColor:colors.border,padding:4,gap:4},sideButton:{flex:1,paddingVertical:14,borderRadius:radius.sm,alignItems:'center'},buyActive:{backgroundColor:colors.accent},sellActive:{backgroundColor:colors.warning},sideText:{color:colors.muted,fontSize:12,fontWeight:'900'},sideActiveText:{color:colors.background},typeGrid:{gap:8},typeButton:{backgroundColor:colors.surfaceElevated,borderRadius:radius.sm,padding:12,borderWidth:1,borderColor:colors.border},typeActive:{borderColor:colors.accent,backgroundColor:colors.accentSoft},typeTitle:{color:colors.text,fontSize:11,fontWeight:'900'},typeActiveText:{color:colors.accent},typeHint:{color:colors.muted,fontSize:9,lineHeight:14,marginTop:3},field:{color:colors.text,backgroundColor:colors.surfaceElevated,borderRadius:radius.sm,padding:13,fontSize:15},errorBox:{backgroundColor:colors.surfaceElevated,borderRadius:radius.sm,padding:10,gap:3},errorText:{color:colors.warning,fontSize:10,lineHeight:15},previewCard:{backgroundColor:colors.surface,borderWidth:1,borderColor:colors.accent,borderRadius:radius.lg,padding:spacing.md,gap:10},previewHeader:{flexDirection:'row',justifyContent:'space-between',alignItems:'center'},previewTitle:{color:colors.text,fontSize:16,fontWeight:'900'},day:{color:colors.accent,fontSize:9,fontWeight:'900'},row:{flexDirection:'row',justifyContent:'space-between',alignItems:'center',paddingVertical:4},rowValue:{color:colors.text,fontSize:11,fontWeight:'800'},previewMessage:{backgroundColor:colors.surfaceElevated,borderRadius:radius.sm,padding:10,gap:3},success:{color:colors.accent,fontSize:10,fontWeight:'800'},previewButton:{backgroundColor:colors.accentSoft,borderRadius:radius.sm,paddingVertical:13,alignItems:'center'},previewButtonText:{color:colors.accent,fontSize:11,fontWeight:'900'},submitButton:{backgroundColor:colors.accent,paddingVertical:14,borderRadius:radius.sm,alignItems:'center'},submitDisabled:{opacity:0.45},submitText:{color:colors.background,fontSize:11,fontWeight:'900'},executionNote:{color:colors.muted,fontSize:9,lineHeight:14},note:{backgroundColor:colors.accentSoft,borderRadius:radius.md,borderWidth:1,borderColor:colors.border,padding:spacing.md},noteTitle:{color:colors.accent,fontSize:10,fontWeight:'900'},noteText:{color:colors.muted,fontSize:9,lineHeight:15,marginTop:5}
});

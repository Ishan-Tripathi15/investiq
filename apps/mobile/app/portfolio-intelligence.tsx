...
type PortfolioAction = { id: string; severity: 'critical' | 'high' | 'medium' | 'low'; title: string; reason: string; action: string; source: 'portfolio' | 'allocation' | 'goal'; amount?: number };
type ActionCenter = { actions: PortfolioAction[]; summary: { critical: number; high: number; medium: number; low: number } };import { useCallback, useEffect, useState } from 'react';
...
export default function PortfolioIntelligenceScreen() {
  ...
  const load = useCallback(async (refresh = false) => {
    ...
    try {
      ...
      const actionResponse = await fetch(`${API_URL}/portfolio/action-center`, { headers: { authorization: `Bearer ${token}` } });
      if (actionResponse.ok) setActionCenter(await actionResponse.json() as ActionCenter);
  ...
  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => void load(true)} />}>
    ...
    {error ? <View style={styles.card}><Text style={styles.cardTitle}>Intelligence unavailable</Text><Text style={styles.muted}>{error}</Text></View> : report && <>
      ...
      {actionCenter && <Section title="Action Center"><View style={styles.card}><View style={styles.actionSummary}><Text style={styles.muted}>Critical {actionCenter.summary.critical}</Text><Text style={styles.muted}>High {actionCenter.summary.high}</Text><Text style={styles.muted}>Medium {actionCenter.summary.medium}</Text><Text style={styles.muted}>Low {actionCenter.summary.low}</Text></View>{actionCenter.actions.slice(0, 5).map((item) => <View key={item.id} style={styles.actionCard}><View style={styles.actionHead}><Text style={styles.rowTitle}>{item.title}</Text><Text style={styles.severity}>{item.severity.toUpperCase()}</Text></View><Text style={styles.muted}>{item.reason}</Text><Text style={styles.rowText}>{item.action}</Text></View>)}</View></Section>}

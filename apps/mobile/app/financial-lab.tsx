...
function growth(first?: number, last?: number) { return first == null || last == null || first <= 0 ? undefined : (Math.pow(last / first, 1 / Math.max(1, 1)) - 1) * 100; }
...
export default function FinancialLab() {
  ...
  const revenueGrowth = first && last ? growth(first.revenue, last.revenue) : undefined;
  const profitGrowth = first && last ? growth(first.netIncome, last.netIncome) : undefined;
  ...
  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    ...
    <Text style={styles.subtitle}>Read company fundamentals separately from price action.</Text>
    ...
    {data && <>
      ...
      <View style={styles.grid}><Metric label="Revenue change" value={pct(revenueGrowth)} meta="First to latest reported period"/><Metric label="Profit change" value={pct(profitGrowth)} meta="First to latest reported period"/><Metric label="Latest operating margin" value={pct(last?.operatingMarginPct)} meta="Latest reported period"/><Metric label="Latest net margin" value={pct(last?.netMarginPct)} meta="Latest reported period"/></View>

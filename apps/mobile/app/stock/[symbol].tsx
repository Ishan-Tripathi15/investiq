...
export default function StockDetailScreen() {
  const { symbol: rawSymbol } = useLocalSearchParams<{ symbol: string }>(); const symbol = (rawSymbol ?? '').toUpperCase();
  ...
  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    ...
    {loading ? <View style={styles.loading}><ActivityIndicator size="large" color={colors.accent}/><Text style={styles.muted}>Loading verified market data…</Text></View> : error ? <View style={styles.error}><Text style={styles.errorTitle}>Stock data unavailable</Text><Text style={styles.muted}>{error}</Text><TouchableOpacity onPress={() => void load()} style={styles.retry}><Text style={styles.retryText}>Retry</Text></TouchableOpacity></View> : <>
      ...
      <View style={styles.actions}><TouchableOpacity onPress={() => void toggleWatchlist()} style={styles.watch} disabled={watchLoading}><Text style={styles.watchText}>{watchLoading ? 'Saving…' : watched ? '★ In watchlist' : '☆ Add to watchlist'}</Text></TouchableOpacity><Link href={`/portfolio-copilot?symbol=${encodeURIComponent(symbol)}`} asChild><TouchableOpacity style={styles.ai}><Text style={styles.aiText}>✦ Ask InvestIQ</Text></TouchableOpacity></Link></View>
      {watchMessage ? <Text style={styles.watchMessage}>{watchMessage}</Text> : null}

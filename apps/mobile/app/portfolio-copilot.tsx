...
type ApiResult = { generatedAt: string; source: { configured: boolean; provider: string; model?: string; message: string }; context: { asOf: string; answerability: 'grounded' | 'insufficient_data'; evidence: Evidence[]; memoryUsed: MemoryUsed[] }; response: CopilotResponse };
...
export default function PortfolioCopilotScreen() {
  ...
  const clearMemory = useCallback(async () => {
    try {
      const token = await getAccessToken();
      if (!token) { router.replace('/login'); return; }
      const response = await fetch(`${API_URL}/portfolio/copilot/memory`, { method: 'DELETE', headers: { authorization: `Bearer ${token}` } });
      if (!response.ok) throw new Error('Unable to clear Copilot memory');

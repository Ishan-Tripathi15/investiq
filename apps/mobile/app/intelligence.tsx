...
type HistoryResponse = { available: boolean; points: PricePoint[]; source: Source; message?: string };
type FundamentalsResponse = { available: boolean; data: { symbol: string; name?: string; currency?: string; exchange?: string; pe?: number; forwardPe?: number; priceToBook?: number; evToEbitda?: number; periods: FinancialPeriodInput[] } | null; source: Source; message?: string };
type ValuationResponse = { available: boolean; points: Array<{ fiscalDate: string; marketCap?: number; pe?: number; pb?: number; ps?: number; evToEbitda?: number; marketCapDate?: string }>; source: Source; message?: string };
...
function ratio(value?: number) { return value == null ? '—' : value.toFixed(2) + 'x'; }
...
export default function Intelligence() {
  ...
  const [valuationHistory, setValuationHistory] = useState<HistoricalValuationPoint[]>([]);
  ...
  async function analyze() {
    ...
    if (!ticker) return;
    setLoading(true); setMessage('Fetching verified price, financial and historical valuation data…'); setFinancialMessage(null); setValuationMessage(null);
    try {
      const from = new Date(Date.now() - 10 * 365.25 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
      const [historyResponse, fundamentalsResponse, valuationResponse] = await Promise.all([
        ...
        fetch(`${API_URL}/market-data/stocks/${encodeURIComponent(ticker)}/valuation-history?from=${from}`),
      ]);
      ...
      const valuation = await valuationResponse.json() as ValuationResponse;
      ...
      setValuationHistory(valuation.available ? valuation.points.map(point => ({ date: point.fiscalDate, ...(point.pe !== undefined ? { pe: point.pe } : {}), ...(point.pb !== undefined ? { pb: point.pb } : {}), ...(point.ps !== undefined ? { ps: point.ps } : {}), ...(point.evToEbitda !== undefined ? { evToEbitda: point.evToEbitda } : {}) })) : []);
      setFinancialMessage(financial.message ?? null); setValuationMessage(valuation.message ?? null);
      ...
      setPoints([]); setFundamentals(null); setValuationHistory([]); setSource(null); setFinancialMessage(null); setValuationMessage(null);

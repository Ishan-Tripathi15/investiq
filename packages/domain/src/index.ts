...
/** Required present value to reach a target under a scenario. */
export function requiredLumpsum(target: number, annualReturnPct: number, years: number): number {
  ...
  return target / Math.pow(1 + annualReturnPct / 100, years);
}

export function projectLumpsum(principal: number, years: number, assumptions: Record<ProjectionScenario, number>): Projection[] {
  return (Object.entries(assumptions) as [ProjectionScenario, number][]).map(([scenario, annualReturn]) => ({
    scenario,
    annualReturn,

  ...
  return horizons.filter((horizon) => horizon > 0).map((horizonDays) => {
    ...
    for (const eventIndex of eventIndices) {
      ...
      while (low <= high) {
        ...
        if (Date.parse(data[middle]!.timestamp) >= target) {
          match = middle;
          high = middle - 1;
        } else low = middle + 1;
      }
    ...
    return {
      ...
      bestReturnPct: outcomes.length ? Math.max(...outcomes) : 0,
      worstReturnPct: outcomes.length ? Math.min(...outcomes) : 0,
    };
  });
}

export function compareAgainstBenchmark(stock: PricePoint[], benchmark: PricePoint[]): RelativePerformance {
  const stockData = ordered(stock);
  const benchmarkData = ordered(benchmark);
  if (stockData.length < 2 || benchmarkData.length < 2) throw new Error('Both series need at least two observations');
  const stockFirst = stockData[0]!;
  const stockLast = stockData[stockData.length - 1]!;
  const benchmarkFirst = benchmarkData[0]!;
  const benchmarkLast = benchmarkData[benchmarkData.length - 1]!;
  ...
  return {
    ...
    stockCagrPct: cagr(stockFirst.close, stockLast.close, stockYears),
    benchmarkCagrPct: cagr(benchmarkFirst.close, benchmarkLast.close, benchmarkYears),
  };
}
...
export function projectScenarios(currentValue: number, years: number, assumptions: { bear: number; base: number; bull: number }): ScenarioProjection[] {
  ...
  return (Object.entries(assumptions) as Array<['bear' | 'base' | 'bull', number]>).map(([name, annualReturnPct]) => ({
    name,
    annualReturnPct,
    projectedValue: currentValue * Math.pow(1 + annualReturnPct / 100, years),
    assumption: `${annualReturnPct.toFixed(2)}% annual return assumption`,
  }));
}

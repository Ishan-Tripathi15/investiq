...
/** Transparent scenario engine. It combines historical evidence with financial quality and valuation adjustments; it is not predictive. */
export function buildScenarioEngine(input: ScenarioEngineInput): ScenarioEngineOutput {
  ...
  const scenarios: ScenarioEngineOutput['scenarios'] = [
    { name: 'bear', annualReturnPct: bear, projectedValue: input.currentValue * Math.pow(1 + bear / 100, input.years), confidence: 'low', assumptions: ['Higher downside volatility than the base case.', quality ? `Financial quality score ${quality.score}/100 is included as a modifier.` : 'No financial quality data supplied.'] },
    { name: 'base', annualReturnPct: base, projectedValue: input.currentValue * Math.pow(1 + base / 100, input.years), confidence: historical != null ? 'medium' : 'low', assumptions: [historical != null ? `Anchored to historical CAGR of ${historical.toFixed(2)}%.` : 'Uses a neutral assumption because price history is unavailable.', quality ? `Financial quality contributes a transparent ${((quality.score - 50) * 0.08).toFixed(2)} percentage-point adjustment.` : 'No financial quality adjustment.', input.pe != null ? `Current P/E supplied: ${input.pe.toFixed(2)}.` : 'No current P/E supplied.'] },
    { name: 'bull', annualReturnPct: bull, projectedValue: input.currentValue * Math.pow(1 + bull / 100, input.years), confidence: 'low', assumptions: ['Favorable return regime relative to the base case.', 'This is an assumption range, not an AI prediction.'] },
  ];
  ...
  const output: ScenarioEngineOutput = { scenarios, warnings };
  if (quality) output.qualityScore = quality.score;
  return output;
}
